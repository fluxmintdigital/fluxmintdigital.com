...
- **Asset risk:** The broader Explorer grammar remains blocked on the 12 exact records in `Explorer_DJ_Production_Frame_Pack_v1/REGENERATION_REQUIRED.md`. Regeneration Pack v1.1 supplies clean standalone illustrations, but normalized transition review shows material style/framing discontinuities with neighboring retained frames, so none were promoted. The complete 12-frame approved idle sequence has now been integrated at Explorer Entry only as a decorative progressive-enhancement slice with responsive delivery derivatives, static/reduced-motion fallback, viewport/visibility suspension, and failure recovery. Wave, blink, head turn, expressions, useful poses, Main Studio, and every Room remain outside the implementation.
