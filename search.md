...
<script id="studio-search-data" type="application/json">
{
  ...
  "artifacts": [{% assign public_artifacts = site.artifacts | where: 'visibility', 'public' %}{% for artifact in public_artifacts %}{% assign artifact_availability = site.data.availability.records | where: 'artifact', artifact.artifact_id | first %}{"title":{{ artifact.title | jsonify }},"description":{{ artifact.description | jsonify }},"type":{{ artifact.artifact_type | jsonify }},"room":{{ artifact.canonical_room | jsonify }},"status":{{ artifact.lifecycle | jsonify }},"availability":{{ artifact_availability.status | default: '' | jsonify }},"series":{{ artifact.series_id | default: '' | jsonify }},"url":{{ artifact.url | jsonify }}}{% unless forloop.last %},{% endunless %}{% endfor %}],
